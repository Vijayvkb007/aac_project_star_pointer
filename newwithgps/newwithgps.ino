...
TinyGPSPlus gps;
SoftwareSerial ss(0, 1); // RX, TX for GPS
...
float Azimuth, altitude, hourangle, latitude = 15.3, longitude = 75.7;
...
void setup()
{
    ...
    resetaltitude();
    Serial.println("What day of the year???, what time is it???");
    while (1)
    {
        if (Serial.available())
        {
...
void loop()
{
    ...
    while (ss.available())
    {
        ...
        if (gps.encode(c))
        {
            ...
            {
                // Use the GPS location information
    ...
    if (altitude < 0)
    {
        Serial.println("The star isn't here now; it's below the horizon as of now. Check for some other time or some other star ..... ");
    }
    else
    {
